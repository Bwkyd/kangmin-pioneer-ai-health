...
export function hasUnapprovedClinicalContent(item: { title?: string; body?: string; metadata?: string }) {
  return UNAPPROVED_CLINICAL_CONTENT.test([item.title ?? "", item.body ?? "", item.metadata ?? ""].join("\n"));
}
...
export function publishProblem(type: ContentType, item: { title?: string; body?: string; version?: number; mediaId?: string | null; metadata?: string }) {
  ...
  if (type === "plan" || hasUnapprovedClinicalContent(item)) return clinicalApprovalRequiredMessage;
