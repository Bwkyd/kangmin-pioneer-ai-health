...
export function createHealthRecordsApi(repository: HealthRecordsRepository, identity: HealthIdentityResolver) {
  ...
  return {
    ...
    saveProfile: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = profileInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "健康档案字段不完整或包含无效值；常见诱因只能由过敏原患者自述记录派生"); const current = await repository.getProfile(userId); const version = current ? parseExpectedVersion(request) : request.headers.has("if-match") ? parseExpectedVersion(request) : 0; const saved = await repository.saveProfile(userId, version, input); const triggers = await repository.listTriggerProjection(userId); return ok({ profile: profilePayload(saved), exposureTriggerProjection: triggers }); } catch (error) { return failed(error); } },
    ...
    saveSymptom: async (request: Request, date: string) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = parseSymptomRecordInput({ date, scores: raw && typeof raw === "object" && !Array.isArray(raw) && "scores" in raw ? (raw as Record<string, unknown>).scores : raw }); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "症状日期或四项评分无效"); const current = (await repository.listSymptoms(userId, date))[0] ?? null; const hasExplicitIdempotencyKey = request.headers.has("idempotency-key"); const replayableCreate = !current || hasExplicitIdempotencyKey; const version = replayableCreate ? 0 : parseExpectedVersion(request); const createKey = replayableCreate ? (hasExplicitIdempotencyKey ? idempotencyKey(request) : `symptom:${date}`) : undefined; const requestHash = createKey ? await hash(input) : undefined; return ok({ record: await repository.saveSymptom(userId, date, version, input, createKey, requestHash) }); } catch (error) { return failed(error); } },
