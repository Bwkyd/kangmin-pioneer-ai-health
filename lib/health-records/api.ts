...
export function createHealthRecordsApi(repository: HealthRecordsRepository, identity: HealthIdentityResolver) {
  ...
  return {
    ...
    saveProfile: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = profileInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "健康档案字段不完整或包含无效值；常见诱因只能由过敏原患者自述记录派生"); const current = await repository.getProfile(userId); const version = current ? parseExpectedVersion(request) : request.headers.has("if-match") ? parseExpectedVersion(request) : 0; return ok({ profile: profilePayload(await repository.saveProfile(userId, version, input)), exposureTriggerProjection: await repository.listTriggerProjection(userId) }); } catch (error) { return failed(error); } },
