...
function idempotencyKey(request: Request, requestHash: string) {
...
function profileInput(value: unknown) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (basic && typeof basic === "object" && !Array.isArray(basic) && typeof record.allergyHistory === "string") {
      ...
      return parseProfileInput({ basicInfo: { displayName, birthDate, sex }, allergyHistory: record.allergyHistory.trim() ? [{ id: null, allergenName: record.allergyHistory, certainty: "unknown", note: null }] : [], ...(record.commonTriggers === undefined ? {} : { commonTriggers: record.commonTriggers }) });
    }
  }
...
export function createHealthRecordsApi(repository: HealthRecordsRepository, identity: HealthIdentityResolver) {
  ...
  return {
    ...
    getProfile: async (request: Request) => { try { const userId = await authenticated(request); return ok({ profile: profilePayload(await repository.getProfile(userId)), exposureTriggerProjection: await repository.listTriggerProjection(userId) }); } catch (error) { return failed(error); } },
    saveProfile: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = profileInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "健康档案字段不完整或包含无效值"); const current = await repository.getProfile(userId); if (raw && typeof raw === "object" && !Array.isArray(raw) && !("commonTriggers" in raw) && current) input.commonTriggers = current.commonTriggers; const version = request.headers.has("if-match") ? parseExpectedVersion(request) : current?.version ?? 0; return ok({ profile: profilePayload(await repository.saveProfile(userId, version, input)), exposureTriggerProjection: await repository.listTriggerProjection(userId) }); } catch (error) { return failed(error); } },
    ...
    createExposure: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = exposureInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "暴露日期、分组选项或其它描述无效；未识别明确因素不能与其它选项同时提交"); const requestHash = await hash(input); const result = await repository.createExposure(userId, idempotencyKey(request, requestHash), requestHash, input); return ok({ record: exposurePayload(result.value) }, result.replayed ? 200 : 201, result.replayed ? { "idempotency-replayed": "true" } : undefined); } catch (error) { return failed(error); } },
    updateExposure: async (request: Request, id: string) => { try { const userId = await authenticated(request); const input = exposureInput(await body(request)); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "过敏原暴露记录字段无效"); const current = (await repository.listExposures(userId, null)).find((item) => item.id === id); if (!current) throw new HealthRecordError(404, "NOT_FOUND", "过敏原暴露记录不存在"); const version = request.headers.has("if-match") ? parseExpectedVersion(request) : current.version; return ok({ record: exposurePayload(await repository.updateExposure(userId, id, version, input)) }); } catch (error) { return failed(error); } },
    deleteExposure: async (request: Request, id: string) => { try { const userId = await authenticated(request); const current = (await repository.listExposures(userId, null)).find((item) => item.id === id); if (!current) throw new HealthRecordError(404, "NOT_FOUND", "过敏原暴露记录不存在"); await repository.deleteExposure(userId, id, request.headers.has("if-match") ? parseExpectedVersion(request) : current.version); return new Response(null, { status: 204 }); } catch (error) { return failed(error); } },
  };
}
