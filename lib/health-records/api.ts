...
function responseHeaders(headers?: HeadersInit) {
  const merged = new Headers(PRIVATE_HEADERS);
  new Headers(headers).forEach((value, key) => merged.set(key, value));
  return merged;
}

function ok(data: unknown, status = 200, headers?: HeadersInit) { return Response.json(data, { status, headers: responseHeaders(headers) }); }
function failed(error: unknown) {
  if (error instanceof HealthRecordError) return Response.json({ ok: false, error: { code: error.code, message: error.message, ...(error.fields ? { fields: error.fields } : {}) } }, { status: error.status, headers: responseHeaders() });
  return Response.json({ ok: false, error: { code: "INTERNAL_ERROR", message: "健康记录服务暂时不可用" } }, { status: 500, headers: responseHeaders() });
}
    ...
    if (basic && typeof basic === "object" && !Array.isArray(basic) && (typeof record.allergyHistory === "string" || Array.isArray(record.allergyHistoryEntries))) {
      ...
      const allergyHistory = Array.isArray(record.allergyHistoryEntries)
        ...
        : typeof record.allergyHistory === "string" && record.allergyHistory.trim()
          ? record.allergyHistory.split(/[、,，\n]/u).map((item) => item.trim()).filter(Boolean).map((allergenName) => ({ id: null, allergenName, certainty: "unknown", note: null }))
          : [];
...
export function createHealthRecordsApi(repository: HealthRecordsRepository, identity: HealthIdentityResolver) {
  ...
  return {
    ...
    saveProfile: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (isRecord(raw) && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); if (isRecord(raw) && typeof raw.allergyHistory === "string" && Array.isArray(raw.allergyHistoryEntries)) throw new HealthRecordError(422, "INVALID_INPUT", "过敏史只能提交结构化记录或旧版文本，不能同时提交两套字段"); const input = profileInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "健康档案字段不完整或包含无效值；常见诱因只能由过敏原患者自述记录派生"); const current = await repository.getProfile(userId); const version = current ? parseExpectedVersion(request) : request.headers.has("if-match") ? parseExpectedVersion(request) : 0; const saved = await repository.saveProfile(userId, version, input); return ok({ profile: profilePayload(saved.profile), exposureTriggerProjection: saved.triggers }); } catch (error) { return failed(error); } },
    ...
    deleteMedication: async (request: Request, id: string) => { try { await repository.deleteMedication(await authenticated(request), id, parseExpectedVersion(request)); return new Response(null, { status: 204, headers: responseHeaders() }); } catch (error) { return failed(error); } },
    ...
    saveSymptom: async (request: Request, date: string) => { try { const userId = await authenticated(request); const raw = await body(request); if (isRecord(raw) && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = parseSymptomRecordInput({ date, scores: isRecord(raw) && "scores" in raw ? raw.scores : raw }); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "症状日期或四项评分无效"); const isUpdate = request.headers.has("if-match"); const createKey = isUpdate ? undefined : request.headers.has("idempotency-key") ? idempotencyKey(request) : `symptom:${date}`; const requestHash = createKey ? await hash(input) : undefined; const version = isUpdate ? parseExpectedVersion(request) : 0; return ok({ record: await repository.saveSymptom(userId, date, version, input, createKey, requestHash) }); } catch (error) { return failed(error); } },
    ...
    createExposure: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (isRecord(raw) && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); if (isRecord(raw) && Array.isArray(raw.factors) && Array.isArray(raw.selections)) throw new HealthRecordError(422, "INVALID_INPUT", "过敏原记录只能提交 factors 或 selections，不能同时提交两套字段"); const input = exposureInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "暴露日期、分组选项或其它描述无效；未识别明确因素不能与其它选项同时提交"); const requestHash = await hash(input); const result = await repository.createExposure(userId, idempotencyKey(request), requestHash, input); return ok({ record: exposurePayload(result.value) }, result.replayed ? 200 : 201, result.replayed ? { "idempotency-replayed": "true" } : undefined); } catch (error) { return failed(error); } },
    updateExposure: async (request: Request, id: string) => { try { const userId = await authenticated(request); const raw = await body(request); if (isRecord(raw) && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); if (isRecord(raw) && Array.isArray(raw.factors) && Array.isArray(raw.selections)) throw new HealthRecordError(422, "INVALID_INPUT", "过敏原记录只能提交 factors 或 selections，不能同时提交两套字段"); const input = exposureInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "过敏原暴露记录字段无效"); const version = parseExpectedVersion(request); return ok({ record: exposurePayload(await repository.updateExposure(userId, id, version, input)) }); } catch (error) { return failed(error); } },
    deleteExposure: async (request: Request, id: string) => { try { const userId = await authenticated(request); const version = parseExpectedVersion(request); await repository.deleteExposure(userId, id, version); return new Response(null, { status: 204, headers: responseHeaders() }); } catch (error) { return failed(error); } },
  };
}
