import { ALLERGEN_GROUPS, HealthRecordError, allergenOption, parseDateFilter, parseExpectedVersion, parseExposureInput, parseMedicationInput, parseProfileInput, type ExposureRecord, type HealthProfile } from "./domain.ts";
...
function profileInput(value: unknown) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (basic && typeof basic === "object" && !Array.isArray(basic) && typeof record.allergyHistory === "string") {
      ...
      return parseProfileInput({ basicInfo: { displayName, birthDate, sex }, allergyHistory: record.allergyHistory.trim() ? [{ id: null, allergenName: record.allergyHistory, certainty: "unknown", note: null }] : [], commonTriggers: record.commonTriggers });
    }
  }
...
export function createHealthRecordsApi(repository: HealthRecordsRepository, identity: HealthIdentityResolver) {
  ...
  return {
    ...
    saveProfile: async (request: Request) => { try { const userId = await authenticated(request); const raw = await body(request); if (raw && typeof raw === "object" && "userId" in raw) throw new HealthRecordError(400, "CLIENT_IDENTITY_FORBIDDEN", "userId 不可由客户端指定"); const input = profileInput(raw); if (!input) throw new HealthRecordError(422, "INVALID_INPUT", "健康档案字段不完整或包含无效值"); const current = await repository.getProfile(userId); const version = request.headers.has("if-match") ? parseExpectedVersion(request) : current?.version ?? 0; return ok({ profile: profilePayload(await repository.saveProfile(userId, version, input)), exposureTriggerProjection: await repository.listTriggerProjection(userId) }); } catch (error) { return failed(error); } },
