...
> ## ✅ Web 运营能力已提交并部署试用环境（2026-08-11 第二十九轮 · 提交 `b8246fb`，PR #193）
...
> 验证：`cd src && npm run check` 全绿（336 tests：260 pass、76 项因未配置 PG/S3 跳过、0 fail，浏览器 E2E PASS）；`python3 scripts/structure-lint.py .`、`git diff --check` 通过。提交 `b8246fb` 已推送分支并创建 Draft PR #193；空库和线上 SQLite 副本在 8788 预检通过，副本迁移 17→20 且 quick_check=ok。停服前备份为 `/srv/kangmin-cli/data/backups/kangmin-mvp-20260811-134712-before-b8246fb.sqlite`，线上原子切换至完整 SHA release，旧版 `e87151d` 保留回滚；公网患者/管理 Web、消息列表、知识问答冒烟及两页构建哈希均通过，服务 active、`NRestarts=0`。上传包和预检库已清理，模型密钥已迁至权限 0600 的环境文件；正式 PostgreSQL/COS 云联调仍需实例与凭据，原生小程序后置。独立工作树未触碰主工作区作者改动；PR 未合并，暂不清理工作树与分支。
