...
> 公网浏览器使用模拟患者的既有已完成会话追问“手册里迎香穴定位在哪里？”，页面实际返回“鼻翼外缘中点旁开约 0.5 寸，鼻唇沟中”及《福建省中医药适宜技术手册》来源，不含斜刺/平刺/针刺；刷新后回答持久化、输入框仍可用，控制台无 warning/error。`cd src && npm run check` 全绿，新增模型波动、知识原文兜底、方案外针刺、真实摘录和刷新恢复回归。仍未取得客户验收；PR 未合并，正式云、原生小程序、整本手册审核与加密配置不在本轮完成范围。
