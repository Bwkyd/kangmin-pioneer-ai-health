...
> ## 🚧 #246 福建手册受控知识导入实现完成，等待 PR/CI 与生产分批启用（2026-08-22 第139轮 · 基线 `main@427d560`）
...
> 验证：最终 `cd src && npm run check` 通过，390 项 Node 测试为 314 通过、0 失败、76 项因未配置 PostgreSQL/S3 按既有条件跳过，真实浏览器 E2E 全路径通过；清单、`git diff --check` 和生产依赖审计通过。worktree 因忽略的私密 vault 不复制导致结构检查缺项；主工作区另有既有过程目录命名问题。本轮尚未写生产数据、提交、推送、合并或部署；生产必须先备份，在候选库创建并索引全部 17 项，复核后统一启用，失败不得部分启用。
