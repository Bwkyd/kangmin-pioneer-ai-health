...
> ## 🔒 按交付策略关闭小程序验收 Issue（2026-08-28 · 基线 `main@472b08a`）
> 按作者最新确认，客户不逐条验收现有任务单；本轮通过 GitHub CLI 将 #385、#386、#387、#388 全部关闭，并在每个 Issue 留下说明：仓内实现、自动化验证和开发版上传工作已完成，客户后续发现具体问题时另开 Issue 跟踪修复。关闭动作是交付管理策略，不等同于逐项客户验收、体验版正式可见或生产发布；此前记录的 `/ready=503`、正式云资源/微信权限、真机与客户实效边界仍然有效。详细操作记录见 [`docs/changes/ops/019_miniprogram-issue-closeout.md`](../docs/changes/ops/019_miniprogram-issue-closeout.md)，通用边界见 [`state/memory/20260828-issue-closeout-acceptance-boundary.md`](memory/20260828-issue-closeout-acceptance-boundary.md)。未修改 `src/`、truth 或作者原件 `hi.md`。
...
> ## ✅ #387/#388 正式链路仓内修复与端到端复核（2026-08-28 · `main@e4cdc94`，PR #402 已合并）
> 按用户确认修复 #387/#388 的可控缺口：消息中心读取并刷新服务端未读数，计数异常不伪装为 0；视频媒体解码或播放失败显示患者可懂提示，不暴露 URL 或平台堆栈。新增正式模式小程序 `request.js` → 本地 HTTP 服务端到端测试，覆盖微信 code 会话、健康授权、症状新增/幂等/修改/CAS/未来日期、日历/趋势、重启后重登恢复、跨患者隔离，以及后台发布文章/视频/通知、媒体字节、未读→已读、下架隐藏。未修改医学 truth、问卷、证型、期别、方案或规则。验证：typecheck、小程序/record/content/shell 冒烟、正式 E2E 2/2、小程序回归与壳 37/37、完整 Node 448 项 370 通过/0 失败/78 因未配置 PostgreSQL/S3 跳过、管理端 6/6、Chromium E2E 通过；远端 quality 4 分 20 秒、image 1 分钟均通过；sequential-thinking 复盘与 `$km-review` 未发现 P0–P2。正式 AppID、合法 HTTPS 域名、服务端微信 AppSecret、SSH/部署、真实已审核内容、iOS/Android 真机和客户验收仍缺，因此 #385–#388 不关闭、不宣称正式交付。
...
> 脱敏密钥扫描在当前 tracked 文件与未忽略新增文件中没有本轮命中；全目录的 230 条历史命中来自测试假 key、忽略区构建产物和只读参考资料，不进入本轮交付。
> 另记录一个非当前运行时 P3：退役 legacy 兼容参考仍有 2 条 PostCSS/Next moderate advisory；依赖修复 dry-run 因网络请求挂起后已停止，未改其锁文件和构建基线，不阻断本轮 `src`/小程序候选。
> 公网只读复核为 `/`、`/admin`、`/live` 200，`/v1/meta` 200，`/ready` 503；503 原因是试用环境未配置加密密钥，当前线上仍是试用 SQLite/本地素材链。按 `$km-review` 复核未发现新的仓内 P0–P2；评审记录见 `docs/reviews/048_overall-launch-readiness-review.md`。仓内候选可进入 PR/CI，但正式整体发车仍受 #385–#388 的正式 PostgreSQL/COS/AES、微信 AppID/权限/AppSecret/合法 HTTPS 域名、真实内容、双平台真机和客户/医学负责人验收阻断。未关闭 Issue、未提交、未推送、未部署，`hi.md` 未修改。

> ## ✅ G1–G3 管理后台缺陷修复与仓内发车复核（2026-08-28 第282轮 · 基线 `main@947e87a`，未提交）
> 按作者确认的推荐完成 G1–G3：知识库草稿按管理员/任务/资料隔离并恢复文本元数据，原始文件不落浏览器且返回后重新选择；全库检索与单资料测试隔离并按 `enabled` fail-closed；文章、视频、站内消息的写入/编辑/发布/下架审计与业务写入共享同一 SQLite/PostgreSQL 事务，已发布公告改稿回草稿，正文附件上传合并等待期间的最新正文；管理员登录按账号与客户端 IP 双维度独立限流，默认不信任任意 `X-Forwarded-For`；会话失效清空旧数据，且丢弃失效前在途刷新结果。
> 新增原子性、限流、草稿恢复、正文合并测试及浏览器回归检查点。完整 `cd src && npm run check` 退出 0：Node 测试 445 条，367 通过、0 失败、78 条因本机未配置 PostgreSQL/S3 跳过；管理端 6/6；真实 Chromium E2E 通过。显式冒烟 core 2/2、content 2/2、record 3/3、shell 6/6、小程序回归 4/4；`check-tools`、清单、`git diff --check` 通过；`structure-lint.py` 仅剩任务前已有的两个 `_work/` 中文目录名问题。
> 按 `$km-review` 三视角复核，本轮未发现新的 P0–P2；未修改问卷、证型、期别、方案、禁忌或 truth。G1–G3 具备仓内代码/测试/CI 发车条件，尚未对 GitHub Issue 做关闭写入。G4–G5（#385–#388）仍不能正式发车：缺正式 AppID/权限、合法 HTTPS 域名、受控微信身份链路、体验版上传、真实内容、双平台真机和客户验收；PostgreSQL/S3 真实适配器仍待环境条件。未提交、未推送、未建 PR、未部署，也未修改 `hi.md`。

> ## 🚦 管理后台缺陷修复可行性实验与发车判断（2026-08-28 第281轮 · 基线 `main@947e87a`，未提交）
> 按作者确认的四项推荐完成有界实验：隔离 SQLite 稳定复现 #390（已发布消息编辑后仍为 `published`）、#394（create/update 没有对应审计事件）；隔离 HTTP 稳定复现 #395（真实 `/v1/admin/session` 连续错误登录不触发限流）；现有 Chromium 后台 E2E 通过。事务探针进一步证明现有 repository 各自开事务，service 外层不能简单嵌套，#394 必须先改成共享事务入口；失败探针未留下数据。
> 结论是具备仓内发车条件：按 G1（#392/#397）→ G2（#390/#391/#393/#394）→ G3（#395/#396）执行，G2 第一停止门为内容与审计同一事务全成全败；不把审计降级为旁路日志。#385–#388 只具备仓内代码/测试准备条件，正式 AppID、微信权限、合法 HTTPS 域名、真实设备、正式内容和客户验收仍是外部阻塞，不提前宣称交付。完整实验记录见 `docs/experiments/041_admin-defect-fixability-smoke.md`。
> 本轮只新增实验记录与状态，不修改 `src/`、truth、数据库、生产环境或 `hi.md`；未提交、未推送、未建 PR、未部署。

> ## 🧪 修复前基线冒烟与开工前元反思（2026-08-28 第280轮 · 基线 `main@947e87a`，未提交）
> 已核对 12 条 Open Issue、对应源码/测试和前一轮 5 个执行组；本地管理后台缺陷的目标、证据和验收条件足够清楚，可以进入实现。重新定义后的边界是：先修复仓内可控的管理后台问题并补齐自动化测试/检查点；小程序先完成可在仓内完成的配置门禁、失败路径和联调准备，真实体验版、客户内容、双平台真机和客户验收继续作为外部门禁，不伪造完成。
> 修复前已运行 `bash scripts/check-tools.sh`、`npm run build --silent`、core smoke（2/2）、content smoke（2/2）、record smoke（3/3）、shell smoke（6/6）和小程序回归检查（4/4），全部通过。它们尚未覆盖 #390–#397 的具体缺陷；`structure-lint.py` 仍只报告任务前已有的两个 `_work/` 中文目录名。
> 实现前仍需作者确认 4 个有方向影响的口径：①表单草稿采用“文本/元数据按管理员与任务自动恢复 + 关闭时确认，原始文件不落浏览器、返回后重新选择”（推荐）；②内容写入与审计事件采用同一数据库事务全成全败（推荐）；③登录限流采用账号与可信客户端 IP 双维度独立计数，默认不信任任意 `X-Forwarded-For`（推荐）；④小程序采用“先完成仓内代码/测试/门禁，外部条件就绪后再真实联调验收”（推荐）。其余问题可按现有 Issue 验收条件直接实现。本轮未修改 `src/`、truth、数据库或生产环境。

> ## 🧭 Open Issue 同类合并与复杂度排序（2026-08-28 第279轮 · 基线 `main@947e87a`，未提交）
> 读取 GitHub 当前 Open Issue 清单，共 12 条：#385–#388、#390–#397；#389 不在开放清单。按“同一实现边界 + 同一验收闭环”合并，形成以下 5 个执行组，并按任务复杂度由低到高排序：
> 1. **G1 · 知识库工作台状态一致性（中低）**：#392、#397。共同边界是 `knowledge-manager` 的任务状态：未保存输入/文件不能静默丢失，单资料命中不能串入全库复核；同一轮完成弹窗恢复、作用域隔离和浏览器/服务端回归。
> 2. **G2 · 管理后台内容写入与编辑一致性（中高）**：#390、#391、#393、#394。共同边界是文章、视频、站内消息的编辑写入链：发布内容修改须回草稿、编辑输入不能因离开或异步上传丢失、create/update 必须可审计；同一轮统一处理状态、并发、恢复、审计与 SQLite/PostgreSQL 回归。
> 3. **G3 · 管理员认证与会话生命周期安全（高）**：#395、#396。共同边界是管理端认证状态：登录失败限流、可信来源取值、过期/撤销/停用后的清缓存和回登录；同一轮完成 HTTP、焦点恢复、倒计时和安全回归。
> 4. **G4 · 小程序正式交付前置（很高，外部阻塞）**：#385。先处理客户 AppID/权限、合法 HTTPS 域名、受控网络与微信身份、体验版上传和双平台真机验收；AppID/AppSecret 不进仓库。
> 5. **G5 · 小程序报价功能正式闭环验收（最高，依赖 G4）**：#386、#387、#388。G4 通过后在同一轮完成助手、症状记录趋势、文章/视频/应用内消息三条报价链的真实联调、真机回归和客户确认；不得用本地 mock、空态或模拟器结果替代正式验收。
> G1–G3 是独立的管理后台问题组，按上列顺序逐组执行；G4 完成后才能进入 G5，G5 内 #386/#387/#388 可在共享正式链路就绪后并行验收。12 条 Open Issue 均保留原编号和状态，本轮只持久化分组与排序到状态板，未关闭、改写或合并 GitHub Issue，未修改 `src/`、truth、数据库或生产环境。

> ## 🐛 管理后台缺陷复查并逐条登记 Issue（2026-08-28 第278轮 · 基线 `main@947e87a`，无代码提交）
> 按 `km-review` 三视角复查 `src/apps/kangmin-admin`、管理 API/core，并以报价 truth 的文章、视频、AI 知识库和站内消息为范围。P0=0，发现 5 条 P1、3 条 P2，已逐条创建并回读 GitHub Issue #390–#397：已发布站内消息编辑后仍保持 published、消息/知识表单无离开保护、正文附件上传的旧闭包覆盖、内容新增/编辑缺少审计事件、管理员 session 登录缺少 strict 限流、会话撤销后缓存仍可见、知识单资料命中串入全库复核。
> 使用隔离 integration SQLite、虚构管理员和 Chromium 动态复现 #390/#391/#392/#396，并查询临时 `audit_events` 支持 #394；#393/#395/#397 由当前代码路径与回归缺口确认。`bash scripts/check-tools.sh`、`cd src && npm run check` 均通过：Node 测试 442 条中 364 通过、78 条因本机未配置 PostgreSQL/S3 跳过、0 失败，管理端 2/2，真实 Chromium E2E 通过。评审记录见 `docs/reviews/047_admin-backend-defect-review.md`。
> 本轮只新增评审、状态和 GitHub Issue，未修改 `src/`、truth、数据库 schema、生产服务或 `hi.md`；未提交、未推送、未创建 PR、未部署。Issue 仍为 OPEN，待作者/实现阶段处理；本轮未新增跨任务长期记忆。

> ## 🟠 小程序报价功能对齐并提交剩余 Issue（2026-08-28 第277轮 · 基线 `codex/miniprogram-e2e-followup-state@5869a6b`）
> 按报价单小程序四项范围复核源码、现有测试、正式服务匿名只读接口和近期微信交付记录：助手页面/问卷/结果卡代码、症状量表/日历/趋势本地体验、学一学/内容详情/消息中心页面及首页导航均已有实现；但当前 `config.js` 仍关闭网络与微信登录，健康记录只落本地缓存，消息本地固定为空，仓库 AppID 留空且最近 CLI 预览/上传返回 `41002 appid missing`。正式服务可读视频，当前文章发布数为零；这些事实不扩大为在线小程序交付。
> 使用 `km-review` 三视角完成患者可懂性、工程事实和医学安全复核：P0=0，P1=4，P2/P3 无独立新增项。已提交 #385（正式工程/体验版/真机/客户验收）、#386（智能辨证调理助手）、#387（症状评估正式身份与服务端保存）、#388（文章/视频/应用内消息闭环）；#378–#380 不重复开启。评审文档见 `docs/reviews/046_miniprogram-quotation-alignment-review.md`。
> 本轮仅新增评审、状态与 GitHub Issue，不改 `src/`、truth、医学规则、生产服务或 `hi.md`；`bash scripts/check-tools.sh`、小程序工作区 check、miniprogram/record/shell/core/content 冒烟和 `git diff --check` 均通过。四项 issue 均受客户微信前置条件、内容供给或真机验收约束，不能据此宣称报价交付完成；未提交本地 commit、未推送代码、未创建 PR、未部署。

> ## ✅ 小程序 E2E 收尾最终自检与记忆更新（2026-08-28 第275轮 · 基线 `main@70e8c77`）
> 开工前已重新 fetch 并核对真实状态：`main` 与 `origin/main` 同为 `70e8c77973c0ba68e626c26951ce6abbe00f58f0`，仅作者原件 `hi.md` 保持未跟踪且未修改；任务 worktree、本地任务分支和远端 `codex/miniprogram-e2e-*` 分支均已清零；开放 Issue 仅剩既有 Dependabot，#378–#380 已关闭，PR #383 已合并且 `quality`/`image` 成功。
> 本轮按收尾检验标准复核状态板、ops/018、相关长期记忆、源码冒烟/完整门禁、Git 分支与工作区；发现并修正状态板把 `e51d7e6` 写成当前主状态的问题，并把“官方 CLI 与显式 `/v2` 参数均返回 `41002` 时应归为账号/AppID 前置条件、不得绕改业务代码”的可复用边界写入微信 CLI 记忆。小程序开发版上传仍未成功，服务器只读复核与本轮既有测试结论不扩大为体验版、真机、审核、客户验收或医学批准。

> ## 🟠 小程序正式交付阻塞项对账（2026-08-28 第276轮 · 基线 `main@64a9639`）
> PR #384 已 squash 合并为 `64a96393a9b0ab1cd97310c527558ad8fab9d56b`，合并后重新 fetch 的 `main` 与 `origin/main` 同步；任务分支与 worktree 仍按收尾标准清零，`hi.md` 仍是唯一未跟踪且未修改的作者原件。#378–#380 已关闭，作者随后建立 #385 追踪正式 AppID/合法域名、体验版上传、真实 iOS/Android 和客户验收；该 Issue 已标记 blocked/risk-high，证据充分但依赖外部条件，不能关闭或宣称交付完成。
> #385 与本轮已修复回归分开管理：客户端当前无合法域名时继续安全降级；待客户确认身份与域名、受控打开正式链路并完成体验版/真机验收后，才重跑对应门禁并关闭。其余服务、测试、文档与记忆结论不变。
...
> ## 🧩 管理后台内容叶子视图形成稳定边界（2026-08-27 第264轮 · 基线 `origin/main@056df24`，待提交）
> 本轮对应 #366，只处理 `AdminApp`、共享契约、共享 UI 和内容叶子视图四个实体。正文编辑、素材绑定、患者预览、内容表格迁入 `features/content-views.tsx`；通用分页进入 `admin-ui.tsx`，预览响应类型进入 `admin-contracts.ts`。内容草稿状态、保存/发布编排仍留在 `ContentManager`，没有借机重写业务或制造知识→内容的反向依赖。
> `AdminApp.tsx` 1156→1004 行，演化护栏同步收紧；文章/视频文案、DOM、上传、预览、API 参数、CSS、权限和医学安全提示不变。后台 typecheck/build、真实 Chromium、`npm run test:acceptance` 与完整 `npm run check` 均退出 0；完整门禁 436 项中 358 通过、78 项因未配置 PostgreSQL/S3 跳过、0 失败。#365 已经 PR #371 两项 CI 全绿并合并关闭，根工作区作者改动仍未触碰。
...
> 集成包独立 typecheck/build/依赖门禁通过；拆空旧目录后发现数据库护栏错误假设旧目录必然存在，已修为存在时才查残留。适配器窄测 75 项中 67 通过、8 项因未配置 S3 跳过；四组冒烟 2/2、2/2、3/3、6/6；完整本地门禁 358 通过、78 条 PostgreSQL/S3 外部资源测试跳过、0 失败，后台 2 条与真实 Chromium E2E 通过。Docker builder/runner 均已携带 integrations workspace，防止运行时软链接缺目标。
> sequential-thinking 六步元反思与 `km-review` 三视角 P0–P2 为 0。演化复诊显示代码文件 306、p50 121、超 600 行占比 14.4%、用例密度 6.3 不变；`src` 顶层 10→9 并同步收紧护栏，导航成本因新增 workspace 壳 14.06→14.18，故仍按中间态记录。真实 S3、PostgreSQL 全量回归与 OCI 镜像须 PR CI 放行；`npm audit` 的一个既有 high 未混入本次结构迁移处理。详见实验 035、评审 039；待 commit、PR、CI、合并与 #348 收尾。
...
> ## 🧪 首个超长文件拆分实验按停止线撤销（2026-08-27 第235轮 · 基线 `main@6704705`，待提交）
> 选择 #321 中风险最低、仅超限 10 行的 `src/tests/browse-increments.test.ts` 做实验 026。改动前真实构建与单文件 Node 冒烟 12/12 通过；候选按内容/方案、环境、访问/失败三条行为轴拆成 292/190/57 行测试文件和 116 行共享 fixture，原 12 个测试名称与断言不减。首次 typecheck 捕获三个遗漏临时目录 import，修复后窄测 12/12；完整 `cd src && npm run check` 退出 0，类型、架构、小程序检查、Node 全量回归和真实浏览器 E2E 均通过。
> 候选虽然让超 600 行文件从 45 降到 44、占比 14.2%→13.7%、p50 120→119，但导航成本从 13.45 升到 13.46 bit/文件，触发实验停止线；p90 784、用例密度 6.0 和 TypeScript 字面冗余率 38.6%均未改善。已撤销全部测试拆分与 `state/evolution-guard.json` 临时收紧，生产代码从未修改；当前门禁重新通过，指标回到 13.45 / 120 / 14.2%。结论是不为仅超 10 行且仍高度内聚的测试机械增加入口，待出现独立高频变化轴再复诊；该 n=1 结论只记录于 `docs/experiments/026_browse-test-split-smoke.md`，不外推为通用规律或长期记忆。

> ## 🧩 超长文件 3% 目标已拆为独立 Issues（2026-08-27 第234轮 · 基线 `main@6704705`，待提交）
> 核对现有 #305–#312 已承接 10 个最大结构热点后，没有重复建票；新增 #318–#327，分别覆盖应用编排、CLI/HTTP 适配器、传输 E2E、跨切面回归、内容辅助双后端、会话持久化、后台契约测试、健康记录测试、临床规则和小程序助手等 31 个未明确承接的长文件。高风险数据库、核心接口与临床任务均带 `risk-high`，全部带 `task` 与 `needs-triage`，Issue 正文均包含范围、禁止项和可执行验收。
> 总追踪 Issue #328 将原始基线 45/318（14.2%）拆为 ≤31/10%、≤19/6%、≤9/3% 三个停止点，同时要求绝对数量与占比双达标，禁止靠新增小文件稀释分母；每个切片必须用例不减、外部行为与医学/权限语义不变、导航成本和 p90 不恶化，并在完成后复诊。`legacy/` 三个长文件与 PostgreSQL 迁移是否作为最终预算内保留项尚未自动豁免，须届时逐项登记理由、责任人、期限和验收。已逐条回读 #318–#328，均为 open、正文非空且标签符合预期；本轮只执行 GitHub Issue 外部写入和状态记录，未改产品代码、truth、医学规则或部署。

> ## 🩺 演化指标完成第二次复诊（2026-08-27 第233轮 · 基线 `main@6704705`，待提交）
> 按 `aias-meta-evolve` 在根工作区重跑体检，和 00:45 观测点相比，两把主尺与两个哨兵全部持平：用例密度 6.0 个/千行、导航成本 13.45 bit/文件、TypeScript 字面冗余率 38.6%、代码文件 p50 120 行；尾部 p90 784 行、最长 2012 行、超 600 行占比 14.2%，最宽目录仍为顶层 16 个。结论是演化门禁合并后结构与可运行行为均未退化，但本轮也没有新增业务信息量；冗余率只作为同仓时间序列代理，不据此授权删测试或重构。
> 正常护栏检查 704 个入库文件、319 个代码文件后通过；随后暂存 8 子目录探针，护栏同时拦下根目录宽度 19>存量基线 18 和新目录宽度 8>上限 7，退出码 1。探针已从索引和工作区完整撤销，`hi.md` 保持未跟踪、未修改。本轮仅重算 `state/evolve/20260827/trend.md` 与 `trend.json` 并记录状态，未改产品、truth、医学规则或外部环境；没有形成需追加长期记忆的新模式。
