...
- [Issue 关闭与验收边界](20260828-issue-closeout-acceptance-boundary.md) — 任务单管理关闭不等于客户、真机、体验版或生产验收；关闭时记录原因、证据、外部边界和后续反馈分流
- [微信小程序上传分层验收与 CLI 服务端口收尾](20260828-wechat-cli-upload-and-closeout.md) — 区分工程检查、上传接口成功与真机/体验版/审核验收，隔离项目身份配置，复用单一预览窗口并安全关闭本机服务端口；显式 `/v2` 仍报 41002 时按账号/AppID 前置条件处理
...
- [部署目标与运行路径先核实](20260828-deploy-target-verify.md) — 校验 SSH 身份三元组，从 systemd 解析运行路径，并显式处理非交互 PATH、workspace bin 与 zsh 特殊变量
...
- [快速冒烟必须证明目标实际执行](20260827-smoke-selector-fail-closed.md) — 多文件测试零命中可能伪绿；逐项命中、失败传播和外部适配器边界必须有证据
