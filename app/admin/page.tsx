...
function ContentTable({ items, onAction, secondaryAction, busy }: { items: Item[]; onAction?: (id: string, action: string) => void; secondaryAction?: string; busy: boolean }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>内容</th><th>分类</th><th>状态</th><th>版本</th><th>更新时间</th>{onAction && <th>操作</th>}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><small>{item.summary || item.source || "暂无摘要"}</small></td><td>{item.category}</td><td><span className={`status ${item.status}`}>{({ draft: "草稿", published: "已发布", offline: "已下架", indexing: "索引中", index_failed: "索引失败" } as Record<string, string>)[item.status] || item.status}</span>{item.clinicalApproval && <small>已审核 v{item.clinicalApproval.contentVersion}</small>}</td><td>v{item.version}</td><td>{new Date(item.updatedAt).toLocaleString("zh-CN")}</td>{onAction && <td><div className="row-actions">{secondaryAction && item.status !== "published" && <button disabled={busy} onClick={() => onAction(item.id, secondaryAction)}>建立索引</button>}{item.status !== "published" && !item.clinicalApproval && <button disabled={busy} onClick={() => onAction(item.id, "approve")}>临床审核</button>}{item.status !== "published" && <button disabled={busy} onClick={() => onAction(item.id, "publish")}>发布</button>}{item.status === "published" && <button disabled={busy} onClick={() => onAction(item.id, "offline")}>下架</button>}</div></td>}</tr>)}</tbody></table></div>;
}
