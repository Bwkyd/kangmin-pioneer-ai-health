...
import { now, runtime } from "@/lib/admin/store";
...
function messageError(error: unknown) {
  if (error instanceof HealthRecordError) return Response.json({ error: { code: error.code, message: error.message } }, { status: error.status, headers: privateHeaders() });
  return Response.json({ error: "消息服务暂不可用" }, { status: 503, headers: privateHeaders() });
}
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ items: rows.results, unread: rows.results.filter((item) => !item.isRead).length }, { headers: privateHeaders() });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id) return Response.json({ error: "消息不存在" }, { status: 404, headers: privateHeaders() });
    ...
    return Response.json({ id, isRead: true }, { headers: privateHeaders() });
  } catch (error) { return messageError(error); }
}
