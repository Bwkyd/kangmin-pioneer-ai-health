...
export async function POST(request: Request) {
  try {
    ...
    const timestamp = now();
    await values.DB.prepare("INSERT INTO plan_steps (id, plan_id, position, title, instruction, media_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(id, planId, Number(body.position) || 1, cleanText(body.title, 160), cleanText(body.instruction, 4000), cleanText(body.mediaId, 120) || null, timestamp, timestamp).run();
    await audit(session.username, "create_step", "plan", planId, { stepId: id });
