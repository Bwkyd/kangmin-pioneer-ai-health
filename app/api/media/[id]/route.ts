...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const media = await values.DB.prepare("SELECT object_key, content_type FROM media_assets WHERE id = ? AND status = 'ready' AND EXISTS (SELECT 1 FROM content_items WHERE media_id = media_assets.id AND status = 'published')").bind(id).first<{ object_key: string; content_type: string }>();
