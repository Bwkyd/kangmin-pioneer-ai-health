...
> 状态：核心、内容与症状管理入口已合并；基础壳入口正在按 #342 单独交付
> 日期：2026-08-27
> 事实基线：`origin/main@6690e696b42419cfd299acfe5ad6f4901d36be68`
...
`cd src && npm run test:smoke:shell`：逐条确认小程序会话、401 重试、未配置拒绝、首页
导航、流式 UTF-8 与环境 fail-closed；目标名漂移、零/部分命中或断言失败均 exit 1。
该命令不替代真实微信身份、真机或 PostgreSQL/S3 契约。

2026-08-28 逐页 E2E 复核另补 `cd src && npm run test:smoke:miniprogram`，逐项检查 #378/API、
#378/文案、#379/结构和 #380/行为；修复前基线按预期 4/4 报红，修复后 4/4 通过，详见计划
016 与实验 040。新增回归用例单独放在 `src/tests/miniprogram-regression.test.ts`，避免继续
增长既有长文件。健康档案实际像素边界和点击反馈仍必须在微信开发者工具 390px 视口复核，
不能由本地源码检查替代；本轮已用匿名本地 fixture 完成该复核。
