...
- 本地服务：此前 `127.0.0.1:3000` 可达；本轮在隔离持久化 D1 中应用 0000–0005 迁移后，以 Worker `127.0.0.1:39999` 完成浏览器验收并已停止；初次未迁移时 `/api/v1/health-records/exposures` 返回 no such table，已修正测试环境初始化，不代表生产 D1 已迁移
...
| 代码测试与 E2E | completed | 授权主机 `npm test` 55/55 PASS；`npm run lint` 0 errors/1 既有 warning；隔离 D1 浏览器 E2E 通过，生产与客户验收分开阻塞 |
| P1 真修与回归 | completed | 结构化过敏史、过敏原目录、If-Match、症状 unknown、#100 入口、错误态和异步响应保护均已实现并有回归测试；待提交冻结新 SHA |
| 三类独立对抗审查 | pending | 旧候选 `75d5bdb...` 的三路 FAIL 只作为修复依据；必须对新的精确候选 SHA 重跑，P0/P1 不为零继续真修 |
