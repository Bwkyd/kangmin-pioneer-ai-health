...
- 本地服务：旧 `127.0.0.1:3000` vinext 进程仍为 HTTP 500，精确原因是未解析新引入的 `@/lib/agent/approved-plans`；本轮用当前构建启动隔离 Wrangler `127.0.0.1:39998`，隔离 D1 的 HTTP/浏览器验收通过后已停止，二者不能混为生产结论
