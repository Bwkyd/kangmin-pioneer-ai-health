...
- **权威资料**：报价及客户确认采用的临床、内容、页面资料统一存放于 `vault/truth/`，是开发与验收唯一真相源，未经授权不得公开或提交。
- **权威顺序**：范围、所属端、价格和工期以 `vault/truth/抗敏先锋AI小程序报价表.md` 为最高依据；报价范围内的题面、证型与期别、调理方法、视频分别由 truth 中对应专项文件裁决。`docs/`、`state/`、代码和测试均不得反向覆盖 truth；同一职责仍冲突或缺失时停止推断，待作者确认并写回 truth，AI 推断没有权威。
