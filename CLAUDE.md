...
**抗敏先锋 AI 鼻健康管理系统（kangmin）**是一款面向过敏性鼻炎患者的中医智能体小程序，围绕鼻健康评估、症状与健康记录、个性化调养内容和持续健康管理，为患者提供服务，并为运营人员提供配套的内容与用户管理能力。

系统采用 CLI-first 架构：核心业务能力先沉淀为可测试、可组合的命令与应用服务，再由 HTTP 接口和小程序等前端承载。涉及评估和调养内容时，以 `vault/truth/` 中客户确认采用的材料及其对应确定性规则为开发和验收依据；AI 不能脱离这些材料自行生成结论。

## 项目状态入口

- **项目状态**：每轮实际变化、验证、遗留、待办和 commit 以 `state/board.md` 为唯一真相源，并按倒序追加，供 Agent 跨会话接续。
- **商务资料**：合同、付款凭证、报价及功能清单存放于 `vault/raw/business/`，未经授权不得公开或提交。
- **决策机制**：需客户拍板的事项详写在 `docs/product/YYYY-MM-DD-<topic>-decision.md`；若影响当前接续，本轮 board 只记录链接和状态，客户回复及实施结果在后续轮次回写。

## 目录速查

- `src/` 当前主实现 | `legacy/` 迁移前系统，仅作需求、行为和验收参考，禁止向 `src/` 导入其业务实现
- `vault/{raw,truth,style}` 三层资料（`raw/` 只读；**真相是角色不是属性**：客户确认版本进入 `truth/` 才成为当前开发和验收依据；`style/` 保存品牌与表达规范）
...
1. **先核实再决策**：动手前检查当前代码、测试、运行状态和权限，只把会实质改变范围或结果的问题交给作者拍板。
2. **状态可接续**：每轮现场写入 board，可复用经验写入 `state/memory/`，计划和长期结论进入对应 `docs/` 分类。
3. **CLI-first**：业务能力优先沉淀为可测试的命令和应用服务，再由 HTTP 与前端复用。
4. **隔离并行**：中高风险任务使用 worktree，并行工作控制在 4 个以内且文件所有权不重叠。
...
- **不自动提交/推送，除非作者明确要求**；`.githooks/` 已入库（新 clone/worktree 先执行 `git config core.hooksPath .githooks`），pre-push 禁止直推 `main`，代码变更一律走 feature branch + PR；远端分支保护状态需单独核验。
- **PR 必须人工评审**；流水线建 PR 即停、绝不自动 merge；评审修订必须 push 回同一 PR 分支（绝不本地 amend 不推）。
